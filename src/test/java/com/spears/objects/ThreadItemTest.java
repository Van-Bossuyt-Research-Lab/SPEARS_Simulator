...
public class ThreadItemTest {
    ...
    public void testStates(){
        ThreadItem item = new ThreadItem("item", 10, 0, threadMock);
        item.suspend();
        assertEquals(ThreadItem.STATES.SUSPENDED, item.getState());
        item.setRunning(true);
        assertEquals(ThreadItem.STATES.SUSPENDED, item.getState());
        item.unSuspend();
        assertEquals(ThreadItem.STATES.RUNNING, item.getState());
        item.grantPermission();
        assertEquals(ThreadItem.STATES.RUNNING, item.getState());
        item.setRunning(false);
        assertEquals(ThreadItem.STATES.PERMISSION, item.getState());
        item.markFinished();
        assertEquals(ThreadItem.STATES.COMPLETE, item.getState());
        item.revokePermission();
        item.reset();
        assertEquals(ThreadItem.STATES.WAITING, item.getState());
    }
