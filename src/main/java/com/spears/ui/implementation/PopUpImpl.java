...
class PopUpImpl extends JDialog implements PopUp {
	...
	public int showOptionDialog(String[] options){
		...
		this.add(input3);
		currentConfig = Buttons.OK_CANCEL_OPTIONS;
		placeButtons();
		openWindow();
		while (buttonClicked == null){
			try{
		...
		this.remove(input3);
		this.add(defaultText);
		if (buttonClicked == Options.OK_OPTION){
	...
	private void buttonClick(int which){
		...
		case 0:
			switch (currentConfig){
			case YES_NO_CANCEL_OPTIONS:
				buttonClicked = Options.CANCEL_OPTION;
				break;
			}
			break;
		case 1:
			switch (currentConfig){
			case OK_CANCEL_OPTIONS:
				buttonClicked = Options.OK_OPTION;
				break;
			case YES_NO_OPTIONS:
				buttonClicked = Options.YES_OPTION;
				break;
			case YES_NO_CANCEL_OPTIONS:
				buttonClicked = Options.NO_OPTION;
				break;
			}
			break;
		case 2:
