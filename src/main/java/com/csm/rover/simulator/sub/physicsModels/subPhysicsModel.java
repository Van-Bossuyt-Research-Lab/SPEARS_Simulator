...
public void excecute() throws Exception {
		...
		location =+ (speed_x*time_step, speed_y*time_step, speed_z*time_step);
		...
		Access.updatesubLocation(subName, location, theta, phi);
		...
		battery_current = Math.abs(motor_current[L]) + Math.abs(motor_current[R]) + Math.abs(motor_current[F]) + Math.abs(motor_current[B]);
		...
		motor_temp[L] += ((winding_heat_transfer*(winding_temp[L] - motor_temp[L]) - motor_surface_heat_transfer*(motor_temp[L] - Access.getMapTemperatureAtPoint(location))) / motor_thermal_cap) * time_step;
		motor_temp[R] += ((winding_heat_transfer*(winding_temp[R] - motor_temp[R]) - motor_surface_heat_transfer*(motor_temp[R] - Access.getMapTemperatureAtPoint(location))) / motor_thermal_cap) * time_step;
		motor_temp[F] += ((winding_heat_transfer*(winding_temp[F] - motor_temp[F]) - motor_surface_heat_transfer*(motor_temp[F] - Access.getMapTemperatureAtPoint(location))) / motor_thermal_cap) * time_step;
		motor_temp[B] += ((winding_heat_transfer*(winding_temp[B] - motor_temp[B]) - motor_surface_heat_transfer*(motor_temp[B] - Access.getMapTemperatureAtPoint(location))) / motor_thermal_cap) * time_step;
		}

		@Override
		public SubPhysicsModel clone(){
		return new SubPhysicsModel(this);
		}
